...
internal class WalIndexService
{
    ...
    private readonly HashSet<uint> _confirmTransactions = [];
        ...
        foreach (var buffer in _disk.ReadFull(FileOrigin.Log))
        {
            ...
            if (isConfirmed)
            {
                ...
                if (pageType == PageType.Header)
                {
                    ...
                    Buffer.BlockCopy(buffer.Array, buffer.Offset, headerBuffer.Array, headerBuffer.Offset,
                        PAGE_SIZE);
    ...
    private int CheckpointInternal()
    {
        ...
        // async rewrite of the original checkpoint process
        // 1. Channel for Log scanning
        var logScan = Channel.CreateBounded<PageBuffer>(64);
        Task.Run(async () =>
        {
            var writer = logScan.Writer;
            foreach (var buffer in _disk.ReadFull(FileOrigin.Log)) await writer.WriteAsync(buffer);
            writer.Complete();
        });
        // 2. Channel for mapping page
        var mapping = Channel.CreateBounded<PageBuffer>(16);
        Task.Run(async () =>
        {
            var writer = mapping.Writer;
            var reader = logScan.Reader;
            await foreach (var buffer in reader.ReadAllAsync())
            {
                // this should not happen, but if it does, it means there's a zeroed page in the file, just skip it
                if (buffer.IsBlank()) continue;

                // read direct from buffer to avoid create BasePage structure
                var transactionId = buffer.ReadUInt32(BasePage.P_TRANSACTION_ID);

                // only confirmed paged can be written on data disk
                if (!_confirmTransactions.Contains(transactionId)) continue;
                var pageId = buffer.ReadUInt32(BasePage.P_PAGE_ID);
        ...
        _disk.Write(mapping.Reader.ReadAllAsync().ToBlockingEnumerable(), FileOrigin.Data);
