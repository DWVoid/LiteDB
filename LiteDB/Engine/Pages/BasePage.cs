...
internal class BasePage
{
    ...
    public BasePage(PageBuffer buffer, uint pageID, PageType pageType)
    {
        ...
        DEBUG(buffer.Slice(PAGE_HEADER_SIZE, PAGE_SIZE - PAGE_HEADER_SIZE - 1).All(0),
    ...
    public BufferSlice Get(byte index)
    {
        ...
        ENSURE(IsValidPos(position), $"invalid segment position in index footer: {this}/{index}");
        ENSURE(IsValidLen(length), $"invalid segment length in index footer: {this}/{index}");
    ...
    private BufferSlice InternalInsert(ushort bytesLength, ref byte index)
    {
        ...
        ENSURE(FreeBytes >= bytesLength + (isNewInsert ? SLOT_SIZE : 0),
        ...
        {
            throw LiteException.InvalidFreeSpacePage(PageID, FreeBytes,
        ...
        ENSURE(position + bytesLength <= (PAGE_SIZE - (HighestIndex + 1) * SLOT_SIZE),
    ...
    public void Delete(byte index)
    {
        ...
        if (ItemsCount == 0)
        {
            ...
            DEBUG(_buffer.Slice(PAGE_HEADER_SIZE, PAGE_SIZE - PAGE_HEADER_SIZE - 1).All(0),
    ...
    private bool IsValidPos(ushort position) =>
        position >= PAGE_HEADER_SIZE && position < (PAGE_SIZE - FooterSize);

    /// <summary>
    /// Checks if segment length has a valid value (used for DEBUG)
    /// </summary>
    private bool IsValidLen(ushort length) =>
        length > 0 && length <= (PAGE_SIZE - PAGE_HEADER_SIZE - FooterSize);
    ...
    public override string ToString() =>
        $"PageID: {PageID.ToString().PadLeft(4, '0')} : {PageType} ({ItemsCount} Items)";
}